...
Usage of editor:
  ...
  -font value
    	regular, medium, mono, or a filename. Can be specified multiple times to add to the font theme cycler. If a filename is provided, it will automatically register as the "regular" or "mono" alias depending on its properties (the last one provided takes precedence).
  -fontfallback value
...
- `ListDir [-sub] [-hidden=true] [-short=true] [-rel=true] [-reload] [-f <regexp>]... [-exc <regexp>]... [path...]`: lists directories/files
	- `-sub`: lists directory and sub directories
	- `-hidden`: lists directory including hidden; defaults to true (use `-hidden=false` to disable)
