...
func (sc *Scanner) SrcError(err error) error {
	...
	line, col := IndexLineColumn2(sc.Src, sc.Pos)
...
func (scp *ScannerPos) Restore() {
	scp.sc.Pos = scp.Pos
}
func (scp *ScannerPos) IsEmpty() bool {
	return scp.Pos == scp.sc.Pos
}
func (scp *ScannerPos) Len() int {
	start, end := scp.StartEnd()
	return end - start
}
func (scp *ScannerPos) StartEnd() (int, int) {
...
func (scp *ScannerPos) Bytes() []byte {
	start, end := scp.StartEnd()
