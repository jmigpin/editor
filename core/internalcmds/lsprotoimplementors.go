...
func LSProtoImplementors(args *core.InternalCmdArgs) error {
	ed := args.Ed
	...
	erow2.Exec.RunAsync(func(ctx context.Context, rw io.ReadWriter) error {
		...
		locs, err := ed.LSProtoMan.TextDocumentImplementation(ctx, erow.Info.Name(), ta.RW(), ta.CursorIndex())
		if err != nil {
			return err
		}
		...
		fmt.Fprintf(rw, "lsproto implementors:")
		if len(locs) == 0 {
