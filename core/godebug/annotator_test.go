...
func TestAnnotator33(t *testing.T) {
	inout := []string{
		...
		`Σ0 := Σ.IV(u)
	        ...
	        Σ5 := &Struct1{A: Σ1, B: 2}
	        Σ6 := Σ.IV(Σ5)
	        Σ7 := Σ.IU(Σ6, 17, Σ.ILit(Σ.KV(Σ.IVs("A"), Σ3), Σ.KV(Σ.IVs("B"), Σ4)))
	        a := Σ5
	        Σ8 := Σ.IV(a)
...
func TestAnnotator59(t *testing.T) {
	inout := []string{
		...
		`Σ0 := "a"
	        ...
	        Σ4 := Σ.IV(a)
	        Σ.Line(0, 0, 52, Σ.IA(Σ.IL(Σ4), Σ.IL(Σ.ILit(Σ.KV(Σ1, Σ3)))))`,
	}
	testAnnotator1(t, inout[0], inout[1], srcFunc1)
}
...
func TestAnnotator79(t *testing.T) {
	inout := []string{
		...
		`Σ0 := Σ.IV(1)
	        ...
	        a := W{a: 1, b: 2, c: 3}
	        Σ3 := Σ.IV(a)
	        Σ.Line(0, 0, 40, Σ.IA(Σ.IL(Σ3), Σ.IL(Σ.ILit(Σ.KV(Σ.IVs("a"), Σ0), Σ.KV(Σ.IVs("b"), Σ1), Σ.KV(Σ.IVs("c"), Σ2)))))`,
	}
	testAnnotator1(t, inout[0], inout[1], srcFunc1)
}
...
func TestAnnotator87_2(t *testing.T) {
	inout := []string{
		`a <- <-c`,
		`Σ0 := Σ.IV(c)
	        Σ1 := <-c
...
func _TestAnnotatorSet1(t *testing.T) {
	src1 := `
		package pkg1
		import "fmt"
		func main(){a:=1}
